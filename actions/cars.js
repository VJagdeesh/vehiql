...
export const processCarmageWithAI = async (file) => {
  try {
    if (!process.env.GEMINI_API_KEY)
      throw new Error("Gemini API doesn't have required key");
...
`;
    ...
    try {
      ...
      if (missingFields.length > 0) {
        throw new Error(
          `AI response is missing few fields : ${missingFields.join(",")}`
        );
      }
      return {
        success: true,
...
export const addCar = async ({ carData, images }) => {
  try {
    ...
    if (imageUrls.length === 0) {
      throw new Error("Need valid images ");
    }
